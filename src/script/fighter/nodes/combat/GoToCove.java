...
public class GoToCove extends Node {
    ...
    public boolean validate() {
        return Config.getProgressive().isOgress() && !OgressWrapper.CORSAIR_COVE_DUNGEON.contains(Players.getLocal())
                && GEWrapper.hasEquipment() && GEWrapper.hasRunes(Config.getProgressive().getSpell());
    }

    @Override
    public int execute() {
        main.invalidateTask(this);
        if (!Equipment.containsAll(Config.getProgressive().getEquipmentMap().values().toArray(new String[0]))) {
            OgressWrapper.equipEquipment();
        }
